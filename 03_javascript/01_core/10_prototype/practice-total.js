...
function Product(name, price, stock) {
    ...
    this.getInfo = function () {
        return `${this.name}: ${this.price}원, 재고 ${this.stock}개`;
    };
}

const pd1 = new Product("키보드", 50000, 3);
const pd2 = new Product("마우스", 30000, 5);

console.log(pd1.getInfo());
console.log(pd2.getInfo());
...
const st1 = new Student('홍길동', 85);
const st2 = new Student('유관순', 70);

console.log(`${st1.name}: ${st1.getGrade()}`);
console.log(`${st2.name}: ${st2.getGrade()}`);
console.log(st1.getGrade === st2.getGrade);
console.log(Object.hasOwn(st1, 'getGrade'));
...
Member.prototype.introduce = function () {
    ...
    return `${nickname ?? this.name}님은 ${level ?? '일반'} 회원입니다.`;
}
...
console.log(member1.introduce === member2.introduce);