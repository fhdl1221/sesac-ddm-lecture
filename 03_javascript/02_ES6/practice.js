...
let count = 1;
for (const value of searchHistory) {
...
// function printUser({ name: userName, role, nickname = "이름 없음", skills }) {
...
//     console.log(`${userName}님의 역할은 ${role}입니다.`);
// }

function printUser({ name: userName, role }) {
...
const availableProductsNames = availableProducts.map(({ name }) => name);
...
console.log("판매 가능 상품: ", availableProductsNames);
